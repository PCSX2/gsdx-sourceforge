...
GSMergeFX9::GSMergeFX9()
	: m_dev(NULL)
{
}

bool GSMergeFX9::Create(GSDevice9* dev)
{
	m_dev = dev;
	...
	hr = m_dev->CompileShader(IDR_MERGE9_FX, "vs_main", NULL, &m_vs, il, countof(il), &m_il);
...
void GSMergeFX9::Draw(GSTexture9* st, GSVector4* sr, GSTexture9& dt, PSSelector sel, PSConstantBuffer& cb)
{
	HRESULT hr;
	...
	if(!(ps = m_ps.Lookup(sel)))
	{
		...
		hr = m_dev->CompileShader(IDR_MERGE9_FX, "ps_main", macro, &ps);
